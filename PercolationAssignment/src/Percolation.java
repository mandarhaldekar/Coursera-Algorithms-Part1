public class Percolation {
	...
	public Percolation(int N) {
		...
		openSites = new int[N * N]; // Two extra for virtual top and bottom and
									// one extra as we are starting from N=1
		for (int i = 0; i < N * N; i++)
			openSites[i] = 0;
	...
	/**
	 * Checks bounds of indices, opens site(i,j) and connects it with adjacent
	 * open sites
	 * 
	 * @param i
	 *            : row number
	...
	public void open(int i, int j) {
		...
		if (openSites[xyToOneD(i, j)] == 1) // Already open
			...
			if (i == 1) {
				weightedQuickUnionUF.union(xyToOneD(i, j), N * N); // connect to
																	// virtual
																	// top
			}
			if (i == N) {
				weightedQuickUnionUF.union(xyToOneD(i, j), N * N + 1); // connect
																		// to
																		// virtual
																		// top
			}
	...
	public boolean isFull(int i, int j) {
		...
		// Site is full if it is open and connected to virtual top
		if (openSites[xyToOneD(i, j)] == 1
				&& weightedQuickUnionUF.connected(xyToOneD(i, j), N * N))
			return true;
		return false;
